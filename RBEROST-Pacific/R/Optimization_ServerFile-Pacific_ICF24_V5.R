...
# PURPOSE: To allow users to run the postprocessor and eventually the 
#           preprocessor through Shiny
# BY: Cathy Chamberlin and Sam Ennett, earlier versions by Kate Munson and Alyssa Le (ICF)
# DATE: 3/24/2022
###############################################################################

# This file is NOT intended to be run by itself. It is sourced through 
# 02_Optimization_Postprocessing_RunShiny.R along with 
# Optimization_PostprocessingUserInterfaceFile.R to run the shiny app.
# Debugging is best done with browser() inserted in reactive contexts that may 
# be causing problems.
# Lines marked #*# may be changed by the user.
# Much of the code has been moved to 
# Optimiation_ServerFunctions_Postprocessor-Pacific.R. This is in an effort to 
# decrease the size of this file.
# The Preprocessing code will also have to be sourced through this ServerFile 
# when that component is built into the Shiny app as well.
# Similarly, hopefully much of the preprocessor can be turned into functions and 
# relegated to a separate code file.
...
source(
  paste0(
    shinyapplocation, "Optimization_ServerFunctions_Postprocessor-Pacific_ICF24_V5.R"
  )
)
...
server <- function(input, output, session) {
  ...
  sparrow_in_reactive <- eventReactive(
    ...
    {req(input$sparrow_in); read_csvs_using_fread(input$sparrow_in$datapath)}
  )
  ...
  # Urban input
  #@ SE: created urban inputs dataframe on 5-22-24
  urban_in_reactive <- reactive(
    {req(input$urban_input); read_csvs_using_fread(input$urban_input$datapath)
    }
  )
  
  # Road input
  #@ SE: created road inputs dataframe on 5-22-24
  road_in_reactive <- reactive(
    {
      req(input$road_input);
      read_csvs_using_fread(input$road_input$datapath)
    }
  )
  ...
  NEOS_renamed <- eventReactive(
    ...
    valueExpr = {
      req(
        ...
        nrow(point_comid_reactive()) > 0, nrow(sparrow_in_reactive()) > 0,
  ...
  cost_uncertainty_message <- reactive({
    req(
      !is.null(cost_bootstraps), solved()[scen.vis()] == 1, 
  ...
  loading_bootstraps <- reactive({
    ...
    get_loading_bootstraps(
      NEOS_renamed(), scen.vis(), num.scen(), NEOS_end_rows(), 
      Table_start_rows()
    ) 
  })
  ...
  UserSpecs_loadingtargets_munged <- eventReactive(
    ...
    {      
      req(
        nrow(UserSpecs_loadingtargets_reactive()) > 0, 
        solved()[scen.vis()] == 1, NEOS_renamed,
  ...
  loading_uncertainty_message <- reactive({
    req(
      !is.null(loading_bootstraps), solved()[scen.vis()] == 1,
      nrow(loading_bootstraps()) > 3, loading_targets,
  ...
  point_bmp_vec <- reactive(
    {
      req(NEOS_renamed, solved()[scen.vis()] == 1)
      read_postprocessor_point_bmp_vec(NEOS_renamed(), scen.vis()) 
    }
  )
  ...
  point_results <- reactive(
    {
      req(
        NEOS_renamed, NEOS_end_rows, length(point_bmp_vec()) > 0,
        solved()[scen.vis()] == 1
      )
      parse_point_results(
        NEOS_renamed(), scen.vis(), NEOS_end_rows(), point_bmp_vec()
      )
    }
  )
  ...
  urban_results <- reactive(
    {
      req(
        NEOS_renamed, NEOS_end_rows, length(urban_bmp_vec()) > 0,
        solved()[scen.vis()] == 1
      )
      parse_urban_results(
  ...
  road_bmp_vec <- reactive(
    {
      req(NEOS_renamed, solved()[scen.vis()] == 1)
      read_postprocessor_road_bmp_vec(NEOS_renamed(), scen.vis()) 
    }
  )
  
  road_results <- reactive(
    {
      req(
        NEOS_renamed, NEOS_end_rows, length(road_bmp_vec()) > 0,
        solved()[scen.vis()] == 1
      )
      parse_road_results(
        NEOS_renamed(), scen.vis(), NEOS_end_rows(), road_bmp_vec()
      )
    }
  )
  ...
  ag_results <- reactive(
    {
      req(
        NEOS_renamed, NEOS_end_rows, length(ag_bmp_vec()) > 0, 
        solved()[scen.vis()] == 1
      )
      parse_ag_results(
  ...
  ripbuf_results <- reactive(
    {
      req(
        NEOS_renamed, NEOS_end_rows, length(ripbuf_bmp_vec()) > 0, 
        solved()[scen.vis()] == 1
      )
  ...
  graz_bmp_vec <- reactive(
    {
      req(NEOS_renamed, solved()[scen.vis()] == 1)
      read_postprocessor_graz_bmp_vec(NEOS_renamed(), scen.vis()) 
    }
  )
  
  graz_results <- reactive(
    {
      req(
        NEOS_renamed, NEOS_end_rows, length(graz_bmp_vec()) > 0,
        solved()[scen.vis()] == 1
      )
      parse_graz_results(
        NEOS_renamed(), scen.vis(), NEOS_end_rows(), graz_bmp_vec()
      )
    }
  )
  ...
  area <- eventReactive(
    ...
    {
      req(
        sparrow_in_reactive, streamcat_files_all_reactive,
        solved()[scen.vis()] == 1, "huc12" %in% names(sparrow_in_reactive()),
        all(
          diff(
            unlist(
              lapply(streamcat_files_all_reactive(), ncol), use.names = FALSE
            )
          ) == 0
        )
      )
  ...
  urban_area <- eventReactive(
    eventExpr = input$ViewReport,
    {
      ...
      make_urban_areadf(
        sparrow_in_reactive(), km2_to_ac, urban_in_reactive()
      )
    }
  )
  ...
  urban_results_HUCmerge <- reactive(
    {
      req(
        !is.null(urban_results), urban_bmp_vec, urban_area, solved()[scen.vis()] == 1
      )
      merge_urban_results(urban_results(), urban_area(), urban_bmp_vec(), km2_to_ac) 
    }
  )
  ...
  urban_results_HUC <- reactive(
    {
      req(
        urban_results_HUCmerge, length(urban_bmp_vec()) > 0, 
        solved()[scen.vis()] == 1
      )
  ...
  road_area <- eventReactive(
    eventExpr = input$ViewReport,
    {
      ...
      make_road_areadf(
        sparrow_in_reactive(), km2_to_ac, road_in_reactive()
      )
    }
  )
  
  road_results_HUCmerge <- reactive(
    {
      req(!is.null(road_results), road_bmp_vec, road_area, solved()[scen.vis()] == 1)
      merge_road_results(road_results(), road_area(), road_bmp_vec(), km2_to_ac) 
    }
  )
  
  road_total <- reactive(
    {
      req(!is.null(road_results), road_results_HUCmerge, road_bmp_vec, solved()[scen.vis()] == 1)
      calculate_road_total(road_results_HUCmerge(), road_bmp_vec())
    }
  )
  
  road_results_HUC <- reactive(
    {
      req(
        !is.null(road_results),
        !is.null(road_results_HUCmerge), 
        !is.null(road_bmp_vec),
        length(road_bmp_vec()) > 0, 
        solved()[scen.vis()] == 1
      )
      present_road_results_HUC(road_results_HUCmerge(), road_bmp_vec())
    }
  )
  ...
  ripbuf_results_HUCmerge <- reactive(
    {
      req(
        sparrow_in_reactive, ripbuf_bmp_vec, solved()[scen.vis()] == 1, 
  ...
  ripbuf_results_HUC <- reactive(
    {
      req(
        ripbuf_results_HUCmerge, length(ripbuf_bmp_vec()) > 0,
        solved()[scen.vis()] == 1
      )
  ...
  point_plants <- reactive(
    {
      req(
        point_results_wwtp, solved()[scen.vis()] == 1, 
        "Plant_Name" %in% names(point_results_wwtp()), point_bmp_vec
      )
      get_point_plants(point_results_wwtp(), point_bmp_vec()) 
    }
  )
  
    point_results_HUC <- reactive(
    {
      req(
        point_results_wwtp, solved()[scen.vis()] == 1, 
        "Plant_Name" %in% names(point_results_wwtp()), point_bmp_vec
      )
      get_point_HUC(point_results_wwtp(), point_bmp_vec())
    }
  )
  
  graz_results_HUCmerge <- reactive(
    {
      req(
        !is.null(graz_results), area, graz_bmp_vec, solved()[scen.vis()] == 1
      )
      merge_graz_results(graz_results(), area(), graz_bmp_vec(), km2_to_ac)      
    }
  )
  
  graz_results_HUC <- reactive(
    {
      req(
        graz_results_HUCmerge, length(graz_bmp_vec()) > 0,
        solved()[scen.vis()] == 1
      )
      present_graz_results_HUC(graz_results_HUCmerge(), graz_bmp_vec())
    }
  )
  
  graz_total <- reactive(
    {
      req(graz_results_HUCmerge, graz_bmp_vec, solved()[scen.vis()] == 1)
      calculate_graz_total(graz_results_HUCmerge(), graz_bmp_vec()) 
    }
  )
  ...
  #@ SE: Created new preview tables 5-22-24
  output$preview_urban_in_ui <- renderUI(
    {    
      req(urban_in_reactive, input$preview)
      input$preview
      isolate(validate(need(input$urban_input != "", label = "Urban Areas")))
      render_preview_ui(
        urban_in_reactive(), 
        "preview_urban_in",
        "Urban Inputs File", 
  ...
  output$preview_road_in_ui <- renderUI(
    {    
      req(road_in_reactive, input$preview)
      input$preview
      isolate(validate(need(input$road_input != "", label = "road Areas")))
      render_preview_ui(
        road_in_reactive(), 
        "preview_road_in",
        "Road Inputs File", 
  ...
  output$road <- renderTable(
    expr = {
      req(road_total, length(road_bmp_vec()) > 0)
      return(value = road_total())
    }
  )
  
  output$roaddownloadavail <- reactive(
    {req(road_results_HUC); nrow(road_results_HUC()) > 0}
  )
  
  outputOptions(output, "roaddownloadavail", suspendWhenHidden = FALSE)
  
  output$downloadroaddat <- downloadHandler(
    filename = reactive(
      {paste0("road_by_ComID_Scenario", scen.vis(), ".csv")}
    ),
    content = function(file) {write.csv(road_results_HUC(), file)}
  )
  ...
  output$wwtp <- renderTable(
    expr = {
      req(point_plants, length(point_bmp_vec()) > 0)
      return(value = point_plants())
    }
  )
  
  output$pointresultsavail <- reactive(
    {req(point_results_HUC); nrow(point_results_HUC()) > 0}
  )
  
  outputOptions(output, "pointresultsavail", suspendWhenHidden = FALSE)
  
  output$downloadpointdat <- downloadHandler(
    filename = reactive(
      {paste0("Point_by_ComID_Scenario", scen.vis(), ".csv")}
    ),
    content = function(file) {write.csv(point_results_HUC(), file)}
  )
  
  ## Grazing Output
  output$graz <- renderTable(
    expr = {req(graz_total, length(graz_bmp_vec()) > 0); return(value = graz_total())}
  )
  
  output$grazdownloadavail <-reactive(
    {req(graz_results_HUC); nrow(graz_results_HUC()) > 0}
  )
  outputOptions(output, "grazdownloadavail", suspendWhenHidden = FALSE)
  
  output$downloadgrazdat <- downloadHandler(
    filename = reactive(
      {paste0("GrazingBMP_by_ComID_Scenario", scen.vis(), ".csv")}
    ),
    content = function(file) {write.csv(graz_results_HUC(), file)}
  )
  ...
  output$report <- renderUI(
    {
      ...
      isolate(
        validate(
          ...
          need(
            ...
            paste(
              paste0(
                input$streamcat_crop_files_all$name[
                  which(!(input$streamcat_crop_files_all$size < 10000000))
                ],
                collapse = ", "
              ),
              "StreamCat file size(s) detected as unusually large. Please double check inputs."
            )
          )
        )
      )
      ...
      isolate(
        validate(
          ...
          need(
            all(
              c("watershed_name", "adjusted_daily_target_kgperday", "adjusted_annual_target_kgperyr", "TN_or_TP") %in%
          ...
          need(
            all(
              c("comid", "termflag", "IncAreaKm2", "final_frac", "huc12") %in%
          ...
          need(
            ...
            #solved()[scen.vis()] == 1 &
            #isTruthy(
              #tryCatch(loading_bootstraps(), error = function(x) {FALSE})
            #)
          #) {
            #need(
              #tryCatch(
            ...
            need(
              !(
                tryCatch(
                  ...
                  tryCatch(
                    any(is.na(graz_results_HUCmerge())),
                    error = function(x) {FALSE}
                  )
              ),
